...
<div
	class="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card *:data-[slot=card]:shadow-xs @xl/main:grid-cols-2 @5xl/main:grid-cols-4 grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-linear-to-t lg:px-6"
>
	<Card.Root class="@container/card">
		...
		<Card.Footer class="flex-col items-start gap-2 text-sm">
			<div class="flex items-center gap-2 w-full">
				<Button variant="default" size="sm" aria-label="Start growth plan">Start Growth Plan</Button>
				<Badge variant="outline" class="ml-2">Recommended</Badge>
			</div>
